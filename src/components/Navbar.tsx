...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <div className="sticky top-0 z-40 px-2 sm:px-3 lg:px-4 pt-2 sm:pt-3">
      <header className="rounded-2xl border border-white/10 bg-[#0B132B]/70 backdrop-blur-xl shadow-2xl text-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2">
          ...
          {/* Desktop Navigation Links — glass pill container */}
          {navItems.length > 0 && (
            <nav className="hidden lg:flex items-center gap-1 bg-white/5 backdrop-blur-md p-1 rounded-xl border border-white/10 shrink-0">
              {navItems.map(item => (
                <button
                  key={item.id}
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            {role !== 'guest' && (
              <div className="relative">
                ...
                {showNotifications && (
                  <div className="fixed top-20 left-4 right-4 sm:absolute sm:top-auto sm:left-auto sm:right-0 mt-2 sm:w-80 max-w-sm bg-white/95 backdrop-blur-xl text-slate-900 rounded-2xl shadow-2xl border border-slate-200 p-4 z-50 animate-in fade-in zoom-in-95 mx-auto">
            ...
            {role === 'guest' ? (
              ...
              <div className="flex items-center gap-2 bg-white/5 backdrop-blur-md px-2 sm:px-3 py-1.5 rounded-xl border border-white/10">
                {role === 'admin' ? (
        ...
        {/* Mobile Drawer Menu — glass effect, rounded bottom */}
        {role !== 'guest' && mobileMenuOpen && (
          <div className="lg:hidden bg-[#0B132B]/80 backdrop-blur-xl border-t border-white/10 px-4 py-4 pb-6 space-y-2">
            {navItems.map(item => (
              <button
                key={item.id}
                onClick={() => {
                ...
                className={`w-full text-left px-4 py-3.5 rounded-xl text-base font-bold transition-all flex items-center gap-3 min-h-[48px] active:scale-[0.98] ${
                  activeTab === item.id ? 'bg-amber-400 text-slate-950 shadow-md font-extrabold' : 'text-slate-200 hover:text-white hover:bg-white/10'
                }`}
              >
                {item.label}
              </button>
