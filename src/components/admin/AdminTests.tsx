...
export const AdminTests: React.FC = () => {
  ...
  // Active Creation Mode: 'csv' | 'manual' | 'offline'
  const [creationMode, setCreationMode] = useState<'csv' | 'manual' | 'offline'>('csv');
  ...
  const [testType, setTestType] = useState<'live' | 'offline_marks'>('live');
  ...
  // Update total marks automatically when questions change if desired
  ...
  const handleCreateTestSubmit = (e: React.FormEvent) => {
    ...
    if (creationMode === 'offline') {
      ...
      finalQuestions = creationMode === 'csv' ? parsedQuestions : manualQuestions;
      if (finalQuestions.length === 0) {
        alert('Please upload a CSV or add questions for the test paper.');
        return;
      }
    }
    ...
    StorageService.addTest(
      {
        title: title.trim(),
        topic: topic.trim() || 'General Chemistry',
    ...
    setCsvFileName('');
    setStudentMarksMap({});
    alert('🎯 Test successfully hosted & published for the batch!');
  };
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200 mb-2">
            <Zap className="w-3.5 h-3.5 text-amber-500" /> Advanced Test Management System
          </div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">
            Live Tests & Automatic Ranking
          </h2>
          <p className="text-xs sm:text-sm text-slate-500">
            Host live timed exams with CSV question paper generation, instant evaluation, student response sheets, and real-time leaderboards.
          </p>
        </div>
        ...
        <div className="lg:col-span-7 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            ...
            <div className="flex bg-slate-100 p-1 rounded-2xl text-xs font-bold">
              ...
              >
                <FileSpreadsheet className="w-3.5 h-3.5" /> Option 1: CSV Upload
              </button>
              <button
                type="button"
              ...
              >
                <Plus className="w-3.5 h-3.5" /> Option 2: Question Builder
              </button>
              <button
                type="button"
          ...
          <form onSubmit={handleCreateTestSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. JEE Main Mock - Chemical Bonding"
                  value={title}
            ...
            {/* Row 4: Schedule Time & Date */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Scheduled Start Time *</label>
                <input
                  type="datetime-local"
                  required
            ...
            {creationMode === 'csv' && (
              <div className="space-y-4 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => handleCsvTextChange(getSampleChemistryCSV())}
                    className="text-xs font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Load 10 Chemistry Questions Sample
                  </button>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black text-slate-900">
                        Parsed Questions ({parsedQuestions.length} Questions):
                      </span>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                      {parsedQuestions.map((q, idx) => (
                        <div
                          key={q.id}
                          className="p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-1.5 relative group"
                        >
            ...
            {/* OPTION 2: Interactive Question Builder */}
            {creationMode === 'manual' && (
              <div className="space-y-4 pt-2">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                  <span className="text-xs font-black text-slate-900 block">
                    Add Question #{manualQuestions.length + 1}
                  </span>

                  <input
                    type="text"
                    placeholder="Enter question text (e.g. Which compound has sp3d hybridization?)"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="Step-by-step reason or formula..."
                  ...
                  >
                    <Plus className="w-4 h-4" /> Add Question to Test Paper
                  </button>
                </div>
            ...
            {/* OPTION 3: Offline Marks Entry Mode */}
            {creationMode === 'offline' && (
            ...
            >
              ...
              <span>
                {creationMode === 'offline'
                  ...
                  : `Host & Publish Live Exam (${activeQuestions.length} Questions)`}
              </span>
            </button>
          </form>
        </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-100 text-xs">
                      ...
                      >
                        ...
                        <span>Leaderboard ({submissionCount})</span>
                      </button>
