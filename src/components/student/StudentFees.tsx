...
export const StudentFees: React.FC<StudentFeesProps> = ({ student }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Month-by-month table (Desktop/Tablet) */}
      ...
      {/* Month-by-month list (Mobile Card-Based ledger, Visible on mobile only, zero scroll!) */}
      <div className="block md:hidden space-y-4">
        {feeRecords.length === 0 ? (
          ...
          feeRecords.map(record => (
            <div key={record.id} className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Academic Month</p>
                  <p className="text-base font-black text-slate-900 mt-1">{record.month}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Fee Amount</p>
                  <p className="text-base font-black text-indigo-600 mt-1">₹{record.amount.toLocaleString()}</p>
                </div>
              </div>

              <div className="flex items-center justify-between py-3 border-t border-b border-slate-100">
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 block font-semibold uppercase tracking-wider">Status</span>
                  {record.status === 'paid' ? (
                    <span className="px-2.5 py-0.5 bg-emerald-100 text-emerald-800 rounded-full font-bold text-[10px] inline-flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3" /> Paid
                    </span>
                  ) : record.status === 'pending_verification' ? (
                    <span className="px-2.5 py-0.5 bg-amber-100 text-amber-950 rounded-full font-bold text-[10px] inline-flex items-center gap-1 animate-pulse">
                      <Clock className="w-3 h-3 text-amber-600" /> Pending Admin
                    </span>
                  ) : (
                    <span className="px-2.5 py-0.5 bg-red-100 text-red-700 rounded-full font-bold text-[10px] inline-flex items-center gap-1">
                ...
                <div className="text-right space-y-1">
                  <span className="text-[10px] text-slate-400 block font-semibold uppercase tracking-wider">Paid Date / Ref</span>
                  <p className="font-mono text-slate-600 text-[10px] truncate max-w-[140px]">
                    {record.paidDate || record.transactionRef || '—'}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-1 flex justify-end">
                {record.status === 'unpaid' ? (
                  <button
                    onClick={() => handlePayClick(record)}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white font-black text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2"
                  >
                    <QrCode className="w-4 h-4" /> Pay Fees via UPI
                  </button>
                ) : (
                  <div className="flex items-center justify-between w-full gap-2">
                    <button
                      onClick={() => generateFeeReceiptPDF(student, feeRecords, record)}
                      className="flex-1 py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-extrabold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 border border-indigo-200/60 shadow-sm"
                    >
                      <Download className="w-3.5 h-3.5 text-indigo-600" /> Receipt PDF
                    </button>
                    {record.status === 'pending_verification' && (
                    ...
                    {record.status === 'paid' && (
                      <span className="text-emerald-600 font-black text-xs pr-1">✓ Cleared</span>
                    )}
                  </div>
                )}
              </div>
