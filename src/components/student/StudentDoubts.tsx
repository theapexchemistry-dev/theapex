...
import {
  ...
  Bot, Sparkles, Loader2, AlertTriangle, MessageCircle, RefreshCw, Trash2, ShieldAlert, AlertCircle
} from 'lucide-react';
              ...
              doubts.map(d => (
                <div key={d.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100 text-[10px] text-slate-400 font-mono">
                    <span>Asked on: {d.createdAt}</span>
                    <button
                      type="button"
                      onClick={() => setDoubtToDelete(d)}
                      className="flex items-center gap-1.5 px-2.5 py-1 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-slate-200/80 hover:border-red-200 text-xs font-semibold"
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-5 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Confirm permanent deletion from database & teacher portal.</p>
              </div>
            </div>

            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-2 text-xs">
              <div className="flex justify-between items-center text-slate-600 pb-1.5 border-b border-slate-200/60">
                <span className="font-bold text-slate-800">{doubtToDelete.subject}</span>
                <span className="text-[11px] font-mono text-slate-500">{doubtToDelete.createdAt}</span>
              </div>
            ...
            <div className="p-3 bg-amber-50 rounded-2xl border border-amber-200 flex items-start gap-2.5 text-xs text-amber-900 font-medium">
              ...
              <span>
                This doubt will be <strong>permanently removed</strong> from the database and will also disappear from Mr. Subhamoy Mondal's portal.
              </span>
            </div>
